...
# runas /savecred /user:maingto\schedulingUser "powershell d:\bto-backup\windows\db-backups-compress.ps1"
...
$uncompressedFolder = "d:\backups\_uncompressed"
...
foreach ($backup in $backups) {
  ...
  & "$PSScriptRoot\rar.exe" a -ep -ag-YY-MM-DD_HH-MM-SS -df -m3 -ma5 -md256m -mt24 -t -rr -s "$backupFolder\$dbName\$fileName.rar" "$fullName"
  $t2 = Get-Date
